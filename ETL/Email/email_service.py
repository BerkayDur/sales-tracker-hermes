...
def filter_merged_table(merged_table: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(merged_table, pd.DataFrame):
        raise TypeError(
            "merged_table parameter in filter_merged_table must be of type DataFrame.")
    ...
    product_on_sale_filter = merged_table[threshold_filter &
                                          ~prev_price_filter]["is_on_sale"]
...
def get_merged_customer_and_product_reading_table(
    ...
    merged = customer_information.merge(
        product_reading, on=["product_id"], how="left")
...
def group_by_email(
    ...
    if not isinstance(email, str):
        logging.error(
            "email must be a string containing the email to filter by.")
        raise TypeError(
            "email must be a string containing the email to filter by.")
...
<a href="{row_data["url"]}">{row_data["product_name"]}</a> """
    if not row_data["price"] or (isinstance(row_data["price"], (int, float))
        ...
        message += f"""was £{row_data["price"]
                             }, now £{row_data["current_price"]}"""
...
def get_html_unordered_list(
    ...
    if not all(isinstance(content, str) for content in content_to_place_in_list):
        logging.error(
            "Elements in content_to_place_in_list must be of type str.")
        raise TypeError(
            "Elements in content_to_place_in_list must be of type str.")
...
def send_emails(
    ...
    if len(mail_list) == 0:
        logging.error("Customer emails are not verified, exit early.")
        return False
    logging.info("Successfully get list of email-verified customers.")

    logging.info("Start formatting emails for each customer.")
    mail_list = [get_formatted_email(group_by_email(
        merged_data, email)) for email in mail_list]

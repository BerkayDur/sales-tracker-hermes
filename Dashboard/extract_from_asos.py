...
def is_correct_page(soup: BeautifulSoup) -> bool:
    ...
    if not isinstance(soup, BeautifulSoup):
        logging.error('is_correct_page passed a non BeautifulSoup object')
...
def get_product_code_asos(product_data: dict) -> str | None:
    ...
    if product_id:
        logging.info('Scraped asos product code from product_data dict.')
...
def get_product_name_asos(product_data: dict) -> str | None:
    """Returns product ID from the webpage"""
    if not isinstance(product_data, dict):
        logging.error('product_data is not of type dict in get_product_name_asos.')
        raise TypeError('product_info must be of type dict')
    ...
    if product_name:
        logging.info('Scraped asos product name from product_data dict.')
        return product_name

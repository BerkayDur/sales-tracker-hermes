...
from extract_from_patagonia import (
    is_correct_page,
    scrape_product_information,
    get_product_code_patagonia,
    get_product_name_patagonia
)
...
def test_is_correct_page_valid(valid_outputs):
    ...
    assert mock_soup.find.call_args[1]['attrs'] == {'class': 'product-detail'}
...
def test_get_product_code_patagonia_valid_1():
    fake_product_data = {'mpn': 1234}
    assert get_product_code_patagonia(fake_product_data) == '1234'

def test_get_product_code_patagonia_valid_2():
    fake_product_data = {'sku': 5678}
    assert get_product_code_patagonia(fake_product_data) == '5678'

def test_get_product_code_patagonia_valid_3():
    fake_product_data = {'another_id':3}
    assert get_product_code_patagonia(fake_product_data) == None

def test_get_product_code_patagonia_valid_4():
    fake_product_data = {}
    assert get_product_code_patagonia(fake_product_data) == None


@pytest.mark.parametrize('fake_product_data', [[], tuple(), set(), 2234, 34.04])
def test_get_product_code_patagonia_type_error(fake_product_data):
    with pytest.raises(TypeError):
        get_product_code_patagonia(fake_product_data)


def test_get_product_name_patagonia_valid_1():
    fake_product_data = {'name': 'fake'}
    assert get_product_name_patagonia(fake_product_data) == 'fake'


def test_get_product_name_patagonia_valid_3():
    fake_product_data = {'another_id': 'fake3'}
    assert get_product_name_patagonia(fake_product_data) == None

def test_get_product_name_patagonia_valid_4():
    fake_product_data = {}
    assert get_product_name_patagonia(fake_product_data) == None


@pytest.mark.parametrize('fake_product_data', [[], tuple(), set(), 2234, 34.04])
def test_get_product_name_patagonia_type_error(fake_product_data):
    with pytest.raises(TypeError):
        get_product_name_patagonia(fake_product_data)
...
def get_ses_emails(boto_ses_client: ses_client, method: str) -> list[str]:
    ...
    if not is_ses(boto_ses_client):
        logging.error(
            "client passed to get_ses_emails is not a boto3 ses client.")
        return []
    if method == "all":
    ...
    if method == "unverified":
        ...
        all_emails = boto_ses_client.list_identities(
            IdentityType="EmailAddress")["Identities"]
        verified_emails = boto_ses_client.list_verified_email_addresses()[
